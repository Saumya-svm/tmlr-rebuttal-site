...
function stashMath(md, spans) {
  return md.replace(MATH_RE, (_, tex) => `MJXMATH${spans.push(tex) - 1}Z`);
}

function restoreMath(html, spans) {
  return html.replace(TOKEN_RE, (_, i) => {
    try {
      return katex.renderToString(spans[+i], { output: "mathml", throwOnError: true });
    } catch {
      return `<code>${esc(spans[+i])}</code>`;
    }
  });
}
...
function build(srcDir) {
  ...
  return files.map((file) => {
    ...
    return {
      ...
      toc,
      html: restoreMath(wrapTables(marked.parse(md)), spans),
      counts: {
